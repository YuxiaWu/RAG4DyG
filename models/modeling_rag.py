...
# Define the GNN model for graph fusion
class GNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers, dropout_rate=0.5):
        ...
        if n_layers == 1:
            # If there's only one layer, it goes directly from input to output
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        return x  # Return the raw embeddings

# Define the MLP model for MLP fusion
class MLP_custom(nn.Module):
    def __init__(self, input_dim, output_dim, n_layers):
        ...
        if n_layers == 1:
            # Only output layer when n_layers is 1
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.config = config
        self.gnn_fusion = None

    def get_output_embeddings(self):
        return self.lm_head
    
    def get_mlp(self, input_size, output_size, n_layers=1):
        if self.mlp_fusion is not None: 
            return self.mlp_fusion
        self.mlp_fusion = MLP_custom(input_size, output_size, n_layers)
        return self.mlp_fusion

    def get_gnn(self, nfeat, nhid, nclass, n_layers, dropout):
        if self.gnn_fusion is not None:
            return self.gnn_fusion
            # input_dim, hidden_dim, output_dim, n_layers, dropout_rate
        self.gnn_fusion = GNN(nfeat, nhid, nclass, n_layers, dropout) 
        return self.gnn_fusion
    ...
    def _aug(self, batch_seqs):
        def item_crop(seq, length, eta=0.2):
            num_left = math.floor(length * eta)
            crop_begin = random.randint(4, length - num_left)
        ...
        def item_mask(seq, length, gamma=0.5):
            ...
            self.mask_token = self.config.vocab_size-1
            #print('mask_token: ', self.mask_token)
        ...
        for seq, length in zip(seqs, lengths):
            ...
            if length > 1:
                ...
                aug_len = length
            if switch[0] == 0:
                aug_seq, aug_len = item_crop(seq, length, eta=self.config.eta)
            elif switch[0] == 1:
                aug_seq, aug_len = item_mask(seq, length, gamma=self.config.gamma)
