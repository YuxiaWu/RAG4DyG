...
class ArgsParser(object):
    def __init__(self):
        ...
        parser.add_argument(
            "--train_pair_data_file", default=None, type=str, required=False, help="The input training data file (a text file)."
        )
        parser.add_argument(
            "--output_dir",
            type=str,
        ...
        parser.add_argument(
            "--task", default="classification", type=str, help="The task to be trained or fine-tuned."
        )

        parser.add_argument(
            "--threshold", default=0.5, type=float, help="The threshold for classification."
        )
        ...
        parser.add_argument("--k", default=0, type=int, help="number of retrieval nodes")
        parser.add_argument("--retrieval_type", default='inputs', type=str, help="type of retrieval")
        ...
        parser.add_argument("--loss_type", default='cl', type=str, 
        help="the loss function"
        )
                # add the lambda_decay for time decay
        parser.add_argument("--lambda_decay", 
                            type=float, 
                            default=-1, 
                            help="lambda decay for time decay, set -1 only do not consider time"
                            )
        # add alpha for augmentation loss
        parser.add_argument("--alpha",
                            type=float,
                            default=0.5,
                            help="alpha for augmentation loss"
                            )
        # eta, gamma, beta for three kind of augmentation
        parser.add_argument("--eta",
                            type=float,
                            default=0.2,
                            help="eta for augmentation loss"
                            )
        parser.add_argument("--gamma",
                            type=float,
                            default=0.5,
                            help="gamma for augmentation loss"
                            )
        parser.add_argument("--beta",
                            type=float,
                            default=0.2,
                            help="beta for augmentation loss"
                            )
        
        parser.add_argument("--lrdecay", default=0,type=int,  help="lr decay")
        parser.add_argument("--temperature", default=0.07,type=float, help="temperature for CLloss")
        parser.add_argument("--projector", default=0,type=int, help="whether use projector for retrieval")

        parser.add_argument(
            "--mask_file",
            default=None,
            type=str,
            help="pairwise similarity mask.",
        )
        ...
        parser.add_argument(
            "--freeze", action="store_true", help="Whether to freeze the transformer in SimpleDyG"
        )
        parser.add_argument(
            "--model_name_or_path",
            default=None,
        ...
        parser.add_argument("--topK", type=int, default=5, help="topK retrieval results")
        parser.add_argument("--m", type=int, default=10, help="number of tokens for MLP fusion")
